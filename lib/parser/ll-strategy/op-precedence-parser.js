...
class OpPrecedenceParser {
  constructor() {
    ...
    this.run = token => {
      ...
      console.log('=== new token to parser ==='); // eslint-disable-next-line
  ...
  *programGenerator() {
    ...
    statements.push(s); // ; 分割的statement
  ...
  *nextToken() {
    let token; // console.log('queue in nextToken: ')
  ...
  *checkNextToken(name, condition = true) {
    ...
    let result; // console.log(`check ${next.value} === ${name}, condition: ${condition}`)
